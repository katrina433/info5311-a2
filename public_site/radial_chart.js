...
function radialChart(svg, data, title) {
  ...
  const yExtent = d3.extent(data, d => Object.values(d.y).reduce((acc, cur) => acc + cur, 0));
  ...
  Object.keys(data[0].y).forEach((label, i) => {
    const calcInnerRadius = d => Object.values(d.y).slice(0, i).reduce((acc, cur) => acc + cur, 0);
  ...
  bars.append("text")
    ...
    .attr("y", d => -yScale(Object.values(d.y).reduce((acc, cur) => acc + cur, 0)) - 10)
    .attr("text-anchor", "middle")
    .text(d => d.x);
